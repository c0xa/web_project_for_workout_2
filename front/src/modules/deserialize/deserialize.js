    ...
    deserializePerson(accountsUsers) {
        ...
        if (accounts && accounts.length > 0) {
            accounts.forEach(person => {
                ...
                accountsUsers.set(User.hashCode(person[1].login), new User(person[1].login, person[1].password, person[1].firstName, person[1].secondName, person[1].workplace, person[1].ill, person[1].dataIll, person[1].risk, person[1].visitedWorkspace))
            });
        }
